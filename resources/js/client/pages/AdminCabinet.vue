    ...
    <main class="container px-3 px-sm-0 personal-account dt-guide-cabinet">
        ...
        <div class="main row">
            <div class="d-lg-block d-none col-3 pe-5">
                ...
                <div class="side-menu">
                    <button @click="openMenu('Экскурсии','Действующие')" class="personal-account-nav__link
                        menu-item d-flex rounded p-4 mb-2
                        justify-content-between align-items-center dt-btn--hover-blue"
                            :class="{'personal-account-nav__link_active active': activeTitle=='Экскурсии'}"
                    >
                        <span class="menu-item__name blue-hover font-size-09 semibold">Экскурсии</span>
                        <div class="">
                            <i class="fa-solid fa-globe"></i>
                        </div>
                    </button>
                    <button @click="openMenu('Объекты','Действующие объекты')"
                            class="personal-account-nav__link menu-item d-flex rounded p-4 mb-2
                    justify-content-between align-items-center dt-btn--hover-blue"
                            :class="{'personal-account-nav__link_active active': activeTitle=='Объекты'}">
                        <span class="menu-item__name font-size-09 semibold">Мои объекты</span>
                        <div>
                            <i class="fa-solid fa-map-location-dot"></i>
                        </div>
                    </button>
                    <button @click="openMenu( 'Пользователи')"
                            class="personal-account-nav__link menu-item d-flex rounded p-4 mb-2
                    justify-content-between align-items-center dt-btn--hover-blue"
                            :class="{'personal-account-nav__link_active active': activeTitle=='Пользователи'}">
                        <span class="menu-item__name font-size-09 semibold">Пользователи</span>
                        <div>
            ...
            <admin-tours-component v-if="activeTitle === 'Экскурсии'"/>
            <admin-tour-objects-component v-if="activeTitle === 'Объекты'"/>
            <admin-users-component v-if="activeTitle === 'Пользователи'"/>
            <admin-add-tour-component v-if="activeTitle === 'Добавить экскурсию'"
                      @hideAddExcursion="activeTitle='Экскурсии'"/>
            <admin-add-tour-object-component v-if="activeTitle === 'Добавить объект'"
                             @hideAddObject="activeTitle='Объекты'"/>

            <admin-transactions-component v-if="activeTitle === 'Транзакции'"/>
            <admin-edit-tour-component v-if="selected&&activeTitle==='Редактирование экскурсии'"
                       @hideEditExcursion="hide('Экскурсии')"
                       :tour-id="selected.id"/>

            <admin-edit-tour-object-component v-if="selected&&activeTitle==='Редактирование объекта'"
                              @hideEditTourObject="hide('Объекты')"
...
export default {
    ...
    methods: {
        ...
        hide(title) {
            this.selected = null
            this.activeTitle = title || 'Экскурсии'
        },
        toggle() {
