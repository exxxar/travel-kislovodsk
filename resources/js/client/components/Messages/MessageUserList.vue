<template>
    <div v-for="chat in chats" class="messages-head__message row mx-0 px-4 align-items-center"
        ...
        <div class="col align-items-center ps-3 mx-0" @click="selectChat(chat)">
            <span class="messages-head__name semibold font-size-09 lh-1">{{ chat.title }}</span>
            <p class="messages-head__text opacity-40 thin d-none">{{ chat.message.slice(0,15) }}...</p>
            <p class="messages-head__date opacity-40 thin">{{moment(chat.last_message_at).format('YYYY-MM-DD H:m:s')  }}</p>
        </div>
    ...
    <button
        v-if="canLoadMore"
        @click="loadMoreChats">Загрузить еще
    </button>
</template>
<script>
