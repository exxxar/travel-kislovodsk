...
export default {
    ...
    computed: {
        ...mapGetters(['getTransactionsPaginateObject']),
        filteredLinks(){
            if (!this.pagination)
                return [];
    ...
    methods:{
        nextPage(){
            this.eventBus.emit('transaction_page', this.pagination.meta.current_page+1)
        },
        page(index){
            this.eventBus.emit('transaction_page', index)
        },
        prevPage(){
            this.eventBus.emit('transaction_page', this.pagination.meta.current_page-1)
        }
    ...
    mounted() {
        this.eventBus.on('update_transactions_pagination',()=>{
